...
6. **Both `searcher/api/static/api_reference.html` and `searcher/api/static/llms.txt` must be updated in the same change whenever any of the following occur:**
   ...
   - `api_reference.html` is served live at `/api-reference`; `llms.txt` is served at `/llms.txt`. Both are authoritative references for AI agents consuming this API.
